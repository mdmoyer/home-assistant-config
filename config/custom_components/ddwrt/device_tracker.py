...
class DdWrtDeviceScanner(DeviceScanner):
    ...
    def __init__(self, config):
        ...
        url = f"{self.protocol}://{self.host}:{self.port}/Status_Wireless.live.asp"
    ...
    def get_device_name(self, device):
        ...
        if device not in self.mac2name:
            url = f"{self.protocol}://{self.host}:{self.port}/Status_Lan.live.asp"
            data = self.get_ddwrt_data(url)
    ...
    def _update_info(self):
        ...
        url = f"{self.protocol}://{self.host}:{self.port}/Status_{endpoint}.live.asp"
        data = self.get_ddwrt_data(url)
